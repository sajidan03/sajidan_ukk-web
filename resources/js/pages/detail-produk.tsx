...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, relatedProducts }) => {
    ...
    const getProductImage = (produk: Produk, index: number = 0) => {
        if (produk.gambar_produk && produk.gambar_produk.length > 0) {
            const imageIndex = index < produk.gambar_produk.length ? index : 0;
            return `/storage/assets/produk/${produk.gambar_produk[imageIndex].nama_gambar}`;
        }
        return 'data:image/svg+xml;base64,';
    };
    ...
    const nextImage = () => {
        if (product.gambar_produk) {
            setModalImageIndex((prev) =>
                prev === product.gambar_produk!.length - 1 ? 0 : prev + 1
            );
        }
    };
    ...
    const prevImage = () => {
        if (product.gambar_produk) {
            setModalImageIndex((prev) =>
                prev === 0 ? product.gambar_produk!.length - 1 : prev - 1
            );
        }
    };

    // Effect untuk menutup modal dengan ESC key
    React.useEffect(() => {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
                        ...
                        <div className="space-y-4">
                            {/* Main Image */}
                            <div className="aspect-w-1 aspect-h-1 cursor-zoom-in" onClick={() => openModal(selectedImageIndex)}>
                                <img
                            ...
                            {product.gambar_produk && product.gambar_produk.length > 1 && (
                                <div className="grid grid-cols-4 gap-2">
                                    {product.gambar_produk.map((gambar, index) => (
                                        <div
                                            key={gambar.id}
                                            className={`relative cursor-pointer border-2 rounded transition-all duration-200 ${
                                                index === selectedImageIndex
                                                    ? 'border-[#3862a2] ring-2 ring-[#3862a2] ring-opacity-50'
                                                    : 'border-gray-300 hover:border-[#3862a2]'
                                            }`}
            ...
            {/* Image Modal */}
            {isModalOpen && product.gambar_produk && (
                <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4">
                    <div className="relative max-w-4xl max-h-full w-full">
                        ...
                        {/* Main Modal Image */}
                        <img
                            src={`/storage/assets/produk/${product.gambar_produk[modalImageIndex].nama_gambar}`}
                            alt={`${product.nama_produk} ${modalImageIndex + 1}`}
                        ...
                        {product.gambar_produk.length > 1 && (
                            <div className="flex justify-center mt-4 space-x-2">
                                {product.gambar_produk.map((_, index) => (
                                    <button
                                        key={index}
                        ...
                        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded-full text-sm">
                            {modalImageIndex + 1} / {product.gambar_produk.length}
                        </div>
                    </div>
                </div>
            )}
